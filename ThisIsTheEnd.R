###############################################
# THESIS PIPELINE — supervisor-aligned version
###############################################
...
# -------- Load data --------
reviews_df <- read.csv("C:/Users/gouta/OneDrive/Desktop/Μαθήματα MSc Rotterdam/THESIS/Fake Reviews/df.csv")
...
# --- Fake RF importance ---
importance_df_fake <- data.frame(Feature = names(importance_fake),
                                 Importance = as.numeric(importance_fake))

# Clean names
importance_df_fake$Feature <- gsub("^tfidf_", "", importance_df_fake$Feature)
importance_df_fake$Feature <- gsub("depth_log", "depth", importance_df_fake$Feature)
importance_df_fake$Feature <- gsub("readability_norm", "readability", importance_df_fake$Feature)
...
ggplot(importance_df_fake[1:15, ], aes(x = reorder(Feature, Importance), y = Importance)) +
  ...
  coord_flip() +
  labs(
    title = "Top 15 Important Features – RF (Fake)",
    x = "Feature",
    y = "Importance"
  ) +
...
# --- Helpfulness RF importance ---
importance_df_helpful <- data.frame(Feature = names(importance_helpful),
                                    Importance = as.numeric(importance_helpful))

# Clean names
importance_df_helpful$Feature <- gsub("^tfidf_", "", importance_df_helpful$Feature)
importance_df_helpful$Feature <- gsub("depth_log", "depth", importance_df_helpful$Feature)
importance_df_helpful$Feature <- gsub("readability_norm", "readability", importance_df_helpful$Feature)
...
ggplot(importance_df_helpful[1:15, ], aes(x = reorder(Feature, Importance), y = Importance)) +
  ...
  coord_flip() +
  labs(
    title = "Top 15 Important Features – RF (Helpfulness)",
    x = "Feature",
    y = "Importance"
  ) +
  theme_minimal(base_size = 14)
...
p1 <- ggplot(importance_df_fake[1:15, ], aes(x = reorder(Feature, Importance), y = Importance)) +
  geom_bar(stat = "identity", fill = "darkred") +
  coord_flip() +
  labs(title = "Top 15 Important Features – RF (Fake)", x = "Feature", y = "Importance") +
  theme_minimal(base_size = 14)

p2 <- ggplot(importance_df_helpful[1:15, ], aes(x = reorder(Feature, Importance), y = Importance)) +
  geom_bar(stat = "identity", fill = "darkblue") +
  coord_flip() +
  labs(title = "Top 15 Important Features – RF (Helpfulness)", x = "Feature", y = "Importance") +
  theme_minimal(base_size = 14)
...
### ==========================================================================================
### MINIMAL CONFUSION MATRIX PLOTS WITH LARGER TEXT
### ==========================================================================================

# Function to create minimal confusion matrix plot with larger text
create_minimal_cm_plot <- function(predictions, actual, model_name, task_name) {
  ...
  actual_labels <- if(task_name == "Fake Detection") {
    c("0" = "Real", "1" = "Fake")
  } else {
    c("0" = "Not Helpful", "1" = "Helpful")
  }
  
  predicted_labels <- if(task_name == "Fake Detection") {
    c("0" = "Real", "1" = "Fake")
  } else {
    c("0" = "Not Helpful", "1" = "Helpful")
  }
  
  # Create minimal plot with larger text
  p <- ggplot(cm_table, aes(x = Actual, y = Predicted)) +
    geom_tile(fill = "white", color = "black") +
    geom_text(aes(label = Freq), size = 6) +  # Larger numbers
    labs(
      title = paste(model_name, "-", task_name),
      x = "Actual",
      y = "Predicted"
    ) +
    theme_minimal() +
    theme(
      plot.title = element_text(hjust = 0.5, size = 20, face = "bold"),  # Larger title
      axis.text = element_text(size = 16),  # Larger axis labels (Real/Fake)
      axis.title = element_text(size = 18, face = "bold"),  # Larger axis titles (Actual/Predicted)
      panel.grid = element_blank()
    ) +
    scale_x_discrete(labels = actual_labels) +
    scale_y_discrete(labels = predicted_labels)
  
  return(p)
}

### ==========================================================================================
### GENERATE ALL MINIMAL CONFUSION MATRIX PLOTS WITH LARGER TEXT
### ==========================================================================================

# Create and display all plots
cat("=== MINIMAL CONFUSION MATRICES WITH LARGER TEXT ===\n")

## 1. LASSO - Fake Detection
lasso_fake_plot <- create_minimal_cm_plot(
  predictions = pred_class_fake,
  actual = y_test_fake,
  model_name = "LASSO",
  task_name = "Fake Detection"
)
print(lasso_fake_plot)

## 2. LASSO - Helpfulness Prediction
lasso_helpful_plot <- create_minimal_cm_plot(
  predictions = pred_class_helpful,
  actual = y_test_helpful,
  model_name = "LASSO", 
  task_name = "Helpfulness Prediction"
)
print(lasso_helpful_plot)

## 3. Random Forest - Fake Detection
rf_fake_plot <- create_minimal_cm_plot(
  predictions = pred_rf_fake,
  actual = y_test_fake,
...
## 4. Random Forest - Helpfulness Prediction  
rf_helpful_plot <- create_minimal_cm_plot(
  predictions = pred_rf_helpful,
  actual = y_test_helpful,
  model_name = "Random Forest",
  task_name = "Helpfulness Prediction"
)
print(rf_helpful_plot)

## 5. Balanced Random Forest - Fake Detection
rf_bal_fake_plot <- create_minimal_cm_plot(
  predictions = pred_rf_bal_fake,
  actual = y_test_fake,
  model_name = "Balanced RF",
  task_name = "Fake Detection"
)
print(rf_bal_fake_plot)

## 6. Balanced Random Forest - Helpfulness Prediction
rf_bal_helpful_plot <- create_minimal_cm_plot(
  predictions = pred_rf_bal_help,
  actual = y_test_helpful,
  model_name = "Balanced RF",
  task_name = "Helpfulness Prediction"
)
print(rf_bal_helpful_plot)

## 7. XGBoost - Fake Detection
xgb_fake_plot <- create_minimal_cm_plot(
  predictions = pred_xgb_fake,
  actual = y_test_fake,
  model_name = "XGBoost",
  task_name = "Fake Detection"
)
...
## 8. XGBoost - Helpfulness Prediction
xgb_helpful_plot <- create_minimal_cm_plot(
  predictions = pred_xgb_help,
  actual = y_test_helpful,
  model_name = "XGBoost",
  task_name = "Helpfulness Prediction"
)
print(xgb_helpful_plot)

cat("All confusion matrices with larger text displayed.\n")
...
# Fake
plot(lasso_cv_fake, xvar = "lambda", label = TRUE)
title("Fake Detection", line = 2.5)  # move title higher

# Helpful
plot(lasso_cv_helpful, xvar = "lambda", label = TRUE)
title("Helpfulness Prediction", line = 2.5)  # move title higher
...
p1 <- ggplot(oob_df_fake, aes(x = mtry, y = OOB_Error)) +
  geom_line() + geom_point(size = 3) +
  labs(title = "Fake Detection", x = "mtry", y = "OOB Error") +
  scale_y_continuous(limits = c(0.15, 0.20)) +   # set broader range
  scale_x_continuous(breaks = oob_df_fake$mtry) + # show actual tested values
  theme_minimal()

p2 <- ggplot(oob_df_helpful, aes(x = mtry, y = OOB_Error)) +
  geom_line() + geom_point(size = 3) +
  labs(title = "Helpfulness Prediction", x = "mtry", y = "OOB Error") +
  scale_y_continuous(limits = c(0.15, 0.20)) +   # same broad range
  scale_x_continuous(breaks = oob_df_helpful$mtry) +
  theme_minimal()
...
# Fake reviews heatmap
h1 <- ggplot(xgb_res_df_fake, aes(x = factor(max_depth), 
                                  y = factor(nrounds), 
                                  fill = auc_mean)) +
  geom_tile(color = "white") +
  geom_text(aes(label = round(auc_mean, 3)), size = 3) +   # add mean AUC labels
  facet_wrap(~ eta + subsample + colsample_bytree, ncol = 2) +
  labs(title = "Fake Detection", 
       x = "max_depth", 
       y = "nrounds", 
       fill = "Mean AUC") +
  scale_fill_gradient(low = "white", high = "blue") +       # stronger gradient
  theme_minimal()

# Helpfulness prediction heatmap
h2 <- ggplot(xgb_res_df_help, aes(x = factor(max_depth), 
                                  y = factor(nrounds), 
                                  fill = auc_mean)) +
  geom_tile(color = "white") +
  geom_text(aes(label = round(auc_mean, 3)), size = 3) +   # add mean AUC labels
  facet_wrap(~ eta + subsample + colsample_bytree, ncol = 2) +
  labs(title = "Helpfulness Prediction", 
       x = "max_depth", 
       y = "nrounds", 
       fill = "Mean AUC") +
  scale_fill_gradient(low = "white", high = "blue") +       # stronger gradient
...
h1 <- ggplot(xgb_res_df_fake, aes(x = factor(max_depth), 
                                  ...
                                  fill = auc_mean)) +
  geom_tile(color = "white") +
  geom_text(aes(label = round(auc_mean, 3)), size = 3) +
  facet_wrap(~ eta + subsample + colsample_bytree, ncol = 2) +
  labs(title = "Fake Detection", x = "max_depth", y = "nrounds", fill = "Mean AUC") +
...
h2 <- ggplot(xgb_res_df_help, aes(x = factor(max_depth), 
  ...
  geom_text(aes(label = round(auc_mean, 3)), size = 3) +
  facet_wrap(~ eta + subsample + colsample_bytree, ncol = 2) +
  labs(title = "Helpfulness Prediction", x = "max_depth", y = "nrounds", fill = "Mean AUC") +
  scale_fill_gradient(low = "white", high = "steelblue") +
  theme_minimal()
...
# Add task column
xgb_res_df_fake$task <- "fake"
xgb_res_df_help$task <- "helpful"

# Combine
xgb_res_df <- rbind(xgb_res_df_fake, xgb_res_df_help)

# Plot Fake Detection
h1 <- ggplot(
  filter(xgb_res_df, task == "fake"), 
  aes(x = factor(max_depth), y = factor(nrounds), 
      fill = auc_mean, label = sprintf("%.3f\n(±%.3f)", auc_mean, auc_std))
) +
  geom_tile(color = "white") +
  geom_text(size = 3) +
  facet_wrap(~ eta + subsample + colsample_bytree, ncol = 2) +
...
# Plot Helpfulness Prediction
h2 <- ggplot(
  filter(xgb_res_df, task == "helpful"), 
  aes(x = factor(max_depth), y = factor(nrounds), 
      fill = auc_mean, label = sprintf("%.3f\n(±%.3f)", auc_mean, auc_std))
) +
  geom_tile(color = "white") +
...
df <- data.frame(
  Feature_Fake        = c("restaurant","asked","told","waiter","beef",
                          "pork","sandwich","pretty","sweet","cheese"),
  Coefficient_Fake    = c(3.45,3.42,3.31,2.63,2.25,-4.76,-3.16,-2.53,-2.39,-2.21),
  Feature_Helpfulness = c("pretty","came","probably","kind","though",
                          "asked","staff","food","told","recommend"),
  Coefficient_Help    = c(3.41,2.64,2.43,2.10,1.72,-5.96,-5.60,-5.03,-4.84,-4.47)
)

tbl <- df %>%
  gt() %>%
  cols_label(
    Feature_Fake        = "Feature (Fake)",
    Coefficient_Fake    = "Coefficient",
    Feature_Helpfulness = "Feature (Helpfulness)",
    Coefficient_Help    = "Coefficient"
  ) %>%
  fmt_number(c(Coefficient_Fake, Coefficient_Help), decimals = 2) %>%
  # --------- key styling ----------
tab_options(
  table.background.color          = "white",
  data_row.padding                = px(10),        # more space between rows
  ...
  table_body.hlines.style         = "hidden"       # no inner horizontal lines
) %>%
  tab_style(                                        # black rule under header
    style = cell_borders(sides = "bottom",
                         color = "black", weight = px(1)),
    locations = cells_column_labels(everything())
  ) %>%
  tab_style(                                        # bold headers
